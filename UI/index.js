/*
   Created By Rahul Yadav On 01/18/2022
*/
...
con.login(
  "jsforce@keykloud.com",
  "R12345678" + "8dF7pMlUiqTgrlmYbOfVCPL7",
  (err, userInfo) => {
    if (err) {
      console.error(err);
    } else {
...
app.get("/read/:name", (req, res) => {
  var object =req.params.name;
  con.query(`SELECT Id, Name FROM ${object}`, (err, result) => {
    if (err) {
      res.send(err);
    } else {
...
app.get("/create", (req, res) => {
  con.sobject("Account")
    .create({ Name: "Keykloud 124" }, (err, Create_Result) => {
...
app.get("/update", (req, res) => {
  con
    .sobject("Account")
    .update(
      { Id : created_Id, Name: "test 124" },
      (err, updated_Result) => {
        if (err || !updated_Result.success) {
          return console.error(err);
        } else {
          console.log("Updated Record Id: " + updated_Result.id);
